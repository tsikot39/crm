...
import { ErrorDisplay, setGlobalLoginError } from "./ErrorDisplay";
...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Clear field-specific error when user starts typing (but preserve general errors)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Clear any existing errors at the start of submission
    setErrors({});
    // Note: NOT clearing loginError here - only clear it when we actually start the server call
    ...
    try {
      // Clear any previous login errors when starting a new attempt
      setGlobalLoginError("");
      ...
      if (errorMessage.toLowerCase().includes("invalid credentials")) {
        // Use global error that can't be cleared by React batching
        setGlobalLoginError("Invalid Credentials");
      } else if (
        ...
        setGlobalLoginError(errorMessage || "Login failed. Please try again.");
      }
    } finally {
      setIsLoading(false);
